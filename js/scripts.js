...
let xhr = new XMLHttpRequest(),IdSession;
...
xhr.send();
let xhri = new XMLHttpRequest(),ImgSession;
xhri.open('POST', "ajax/get_session.php", false);
xhri.onreadystatechange = function () {
    if (xhri.readyState == 4 && xhri.status == 200) {
        IdSession = JSON.parse(xhri.responseText)
    }
        ...
        if (this.readyState == 4) { //sit le satus et fini 
            if (this.status == 200) { //Requête traitée avec succès. La réponse dépendra de la méthode de requête utilisée.
                ...
                for (let message of messages) { //j'ai un message parmis les messsage
                    ...
                    console.lo
                    if (message.pseudo == IdSession) {
                        // On ajoute le contenu avant le contenu actuel de discussion  
                        discussion.innerHTML += `<div class="container" style="overflow: hidden;"><div class="lodahout my-4 float-right col-12 col-md-6 clo-lg-3" style="background-color: blue; color: white;"><p>  ${message.message}</p><span class="time-right"> ${message.pseudo} : le ${dateMessage.toLocaleString()}</span></div></div>`
                        // On met à jour le lastId
                        lastId = message.id
                    } else {
                        discussion.innerHTML += `<div class="container" style="overflow: hidden;"><div class="lodahout my-4 ml-3 float-left col-12 col-md-6 clo-lg-3" style="background-color: black; color: white;"><p>  ${message.message}</p><span class="time-right"> ${message.pseudo} : le ${dateMessage.toLocaleString()}</span></div></div>`
                        // On met à jour le lastId
                        lastId = message.id
                    }
